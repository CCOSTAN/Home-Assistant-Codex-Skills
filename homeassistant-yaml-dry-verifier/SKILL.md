...
```bash
python <skill-dir>/scripts/verify_ha_yaml_dry.py config/packages/example.yaml --strict
```

Scan a full directory when doing wider cleanup:

```bash
python <skill-dir>/scripts/verify_ha_yaml_dry.py config/packages config/automations
```
