...
Repository reality check (important):
- If the repository already has established templates, use what exists.
...
1. Read the repository's instructions and source-control state before editing:
   - Follow its established direct-edit, branch, or staged-promotion workflow.
   - Preserve unrelated changes and keep rollback safety by minimizing diffs.
...
4. Validate entities and services before editing:
   ...
   - If MCP is unavailable, use repository configuration and official integration documentation; ask the user when an entity or service still cannot be verified.
...
8. Validate in this order:
   - Run the target repository's documented dashboard validators and UI smoke checks when present.
...
- Read `references/dashboard_rules.md` when you need the full constraint set and repository mapping notes.
...
If MCP is not enabled or reachable, never invent entity or service IDs. Verify from repository configuration and official integration documentation, then ask for any remaining unknowns.
